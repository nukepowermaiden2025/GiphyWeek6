...
$(document).on("click", ".choice", function(event) {
  ...
  var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + place + apikey;
  ...
  $.ajax({
    ...
    for (var i = 0; i < results.length; i++) {
      if (results[i].rating !== "r" && results[i].rating !== "pg-13") {
        ...
        placeimg.addClass("gif");//selector for the pausing established
        placeimg.attr({
          //Link those items to the page with a data-img on an attribute
